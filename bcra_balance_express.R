...
hojas <- paste0("serie semanal ", ult)
# Objeto al que se le asignara la información
datos <- NULL
# Limpieza
rm(ult)

# Carga de la base histórica ---------------------------------------------------
load("historico22.RData")
assign("historico", semanal)
rm(semanal)
...
## Combinación dataset histórico y reciente ------------------------------------
semanal <- rbind(historico, semanal)
rm(historico)
